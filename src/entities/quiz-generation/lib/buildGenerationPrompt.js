...
const PROFILE_GUIDANCE = {
  english: '题目内容保持英语学科风格，题干与选项可使用英文，解析与说明优先使用中文，确保 JSON 可直接导入。',
...
function normalizeGenerationParams(subjectMeta, params = {}) {
  ...
  const requestedTypes = Array.isArray(params.questionTypes) && params.questionTypes.length > 0
  ...
  return {
    mode: params.mode || generation.defaultModes?.[0] || 'practice',
