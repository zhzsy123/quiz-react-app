  ...
  items.forEach((item) => {
    if (item.type === 'reading') {
      ...
      item.questions.forEach((question) => {
        rows.push({
          ...
          type: 'objective',
          userValue: readingAnswers[question.id] || '',
    ...
    if (item.answer?.type === 'objective') {
      ...
      rows.push({
        ...
        prompt: item.prompt,
        type: 'objective',
        userLabel: optionLabel(item.options || [], userValue),
        correctLabel: optionLabel(item.options || [], item.answer?.correct || ''),
        rationale: item.answer?.rationale || '暂无解析',
        isCorrect: userValue === item.answer?.correct,
      })
      return
    }
...
export default function HistoryPageV2() {
  ...
  const trendValues = useMemo(() => {
    return filteredAttempts.slice().reverse().map((item) => (item.objectiveTotal ? Math.round((item.objectiveScore / item.objectiveTotal) * 100) : 0))
  }, [filteredAttempts])
  ...
  const summary = useMemo(() => {
    ...
    const averageRate = totalAttempts
      ? Math.round(filteredAttempts.reduce((sum, item) => {
    ...
    <div className="app-shell">
      <div className="container dashboard-page">
        <section className="dashboard-hero left-hero compact-page-hero">
          <div className="section-header-row no-margin">
            ...
            <div className="dashboard-action-row">
              ...
              <Link className="secondary-btn small-btn" to="/wrong-book"><BookX size={16} /> 错题本</Link>
            </div>
          </div>
        </section>

        <section className="profile-card compact-card">
          <div className="section-header-row"><h2><Filter size={18} /> 筛选</h2></div>
        ...
        <section className="stats-grid">
          <article className="metric-card"><div className="metric-head"><Clock3 size={18} /> 历史考试次数</div><div className="metric-value">{summary.totalAttempts}</div></article>
          <article className="metric-card"><div className="metric-head"><BarChart3 size={18} /> 平均正确率</div><div className="metric-value">{summary.averageRate}%</div></article>
          <article className="metric-card"><div className="metric-head"><BookOpen size={18} /> 累计刷题量</div><div className="metric-value">{summary.totalQuestions}</div></article>
          <article className="metric-card"><div className="metric-head"><ListChecks size={18} /> 累计错题</div><div className="metric-value">{summary.totalWrong}</div></article>
        </section>

        <section className="trend-card">
          <div className="section-header-row"><h2><BarChart3 size={18} /> 历史趋势</h2><span className="section-header-tip">最近 {filteredAttempts.length} 次记录</span></div>
          <div className="trend-body"><Sparkline values={trendValues} /></div>
        </section>

        <section className="record-list-card">
          <div className="section-header-row"><h2><ListChecks size={18} /> 明细记录</h2><span className="section-header-tip">按时间倒序排列</span></div>
                ...
                return (
                  <article key={attempt.id} className="record-item record-item-expanded-shell">
                    ...
                    <div className="local-library-actions attempt-action-row">
                      <button className="secondary-btn small-btn" onClick={() => handleEditAttempt(attempt)}><Pencil size={14} /> 编辑记录</button>
                      <button className="secondary-btn small-btn" onClick={() => setExpandedAttemptId(expanded ? null : attempt.id)}>{expanded ? <EyeOff size={14} /> : <Eye size={14} />}{expanded ? '收起作答' : '查看作答'}</button>
                      <button className="danger-btn small-btn" onClick={() => handleDeleteAttempt(attempt)}><Trash2 size={14} /> 删除记录</button>
                    </div>
                    {expanded && (
                      <div className="attempt-answer-panel">
                        {answerRows.length === 0 ? (
                          ...
                          <div className="answer-review-grid">
                            {answerRows.map((row) => (
                              <article key={row.key} className={`answer-review-card ${row.type === 'subjective' ? 'subjective' : row.isCorrect ? 'correct' : 'wrong'}`}>
                                {row.parentTitle && <div className="answer-review-parent">{row.parentTitle}</div>}
                                <div className="answer-review-prompt">{row.prompt}</div>
                                {row.type === 'subjective' ? (
                                  <>
                                    <div className="answer-review-line"><strong>你的作答：</strong>{row.userText || '未作答'}</div>
                                    <div className="answer-review-line"><strong>参考答案：</strong>{row.referenceText || '暂无参考答案'}</div>
                                  </>
                                ) : (
                                  <>
                                    <div className="answer-review-line"><strong>你的答案：</strong>{row.userLabel}</div>
                                    <div className="answer-review-line"><strong>正确答案：</strong>{row.correctLabel}</div>
                                    <div className="answer-review-line"><strong>解析：</strong>{row.rationale}</div>
                                  </>
                                )}
                              </article>
                            ))}
