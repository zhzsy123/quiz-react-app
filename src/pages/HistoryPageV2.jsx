  ...
  items.forEach((item) => {
    if (item.type === 'reading') {
      ...
      item.questions.forEach((question) => {
        rows.push({
          ...
          correctValue: question.answer?.correct || '',
    ...
    if (item.answer?.type === 'objective') {
      ...
      rows.push({
        ...
        correctValue: item.answer?.correct || '',
...
export default function HistoryPageV2() {
  ...
  const handleEditAttempt = async (attempt) => {
    ...
    await updateAttemptRecord(attempt.id, {
            ...
            <div className="record-list">
              {filteredAttempts.map((attempt) => {
                ...
                const rate = attempt.objectiveTotal
                  ...
                  : 0
                const displayTitle = attemptDisplayTitle(attempt)
                  ...
                  <article key={attempt.id} className="record-item record-item-expanded-shell">
                    <div className="record-main">
                      <div className="record-title-row">
                        <div className="record-title">{displayTitle}</div>
                    ...
                    {expanded && (
                      <div className="attempt-answer-panel">
                        {answerRows.length === 0 ? (
                          <div className="local-library-empty">这条旧记录还没有保存完整作答快照。之后的新考试记录会自动保存全部作答内容。</div>
                        ) : (
