        ...
        <section className="dashboard-hero left-hero">
          <div className="section-header-row no-margin">
            ...
            <div className="dashboard-action-row">
              ...
              <Link className="secondary-btn small-btn" to="/exam/english"><BookOpen size={16} /> 进入英语模考系统</Link>
            </div>
          </div>
          <p>当前档案：{activeProfile?.name || '未命名档案'}。这里聚合了本地考试记录中的客观题错题，支持按题型筛选、错题练习与答对后自动移出错题本。</p>
        </section>
          ...
          <div className="section-header-row">
            ...
            <div className="dashboard-action-row">
              {practiceMode ? (
                ...
                <button className="primary-btn small-btn" onClick={() => { setPracticeMode(true); setPracticeIndex(0) }} disabled={filteredWrongItems.length === 0}>
                  <Play size={14} /> 开始刷错题
                </button>
              )}
            </div>
          ...
          <section className="record-list-card wrongbook-practice-panel">
            <div className="section-header-row">
              ...
              <span className="section-header-tip">
                {filteredWrongItems.length > 0 ? `第 ${practiceIndex + 1} / ${filteredWrongItems.length} 题` : '已无待练习错题'}
              </span>
            </div>

            {!currentPracticeItem ? (
              <div className="local-library-empty">当前筛选条件下已经没有待练习错题了。答对的题目会自动从错题本移除。</div>
            ) : (
              <div className="wrongbook-practice-card">
                <div className="wrongbook-card-head">
                  <div>
                    ...
                    <div className="wrongbook-meta">
                      <span>{getSubjectMeta(currentPracticeItem.subject).shortLabel}</span>
                      <span>题型：{currentPracticeItem.category === 'reading' ? '阅读理解' : currentPracticeItem.category === 'cloze' ? '完形填空' : '单项选择'}</span>
                      <span>来源：{currentPracticeItem.latestAttemptTitle || currentPracticeItem.paperTitle}</span>
                    </div>
                  </div>
                  <div className="wrongbook-tags">
                    {Array.isArray(currentPracticeItem.tags) && currentPracticeItem.tags.slice(0, 4).map((tag) => (
                      <span key={tag} className="tag blue">{tag}</span>
                    ))}
                  </div>
                </div>

                {currentPracticeItem.contextTitle && (
                  <div className="wrongbook-context">
                    <strong>{currentPracticeItem.contextTitle}</strong>
                    {currentPracticeItem.contextSnippet ? `：${currentPracticeItem.contextSnippet}` : ''}
                  </div>
                )}
                ...
                {selectedAnswer && selectedAnswer !== currentPracticeItem.correctAnswer && (
                  <div className="analysis-box">
                    <div><strong>正确答案：</strong>{currentPracticeItem.correctAnswerLabel || currentPracticeItem.correctAnswer || '--'}</div>
                    <div><strong>解析：</strong>{currentPracticeItem.rationale || '暂无解析'}</div>
                  </div>
                )}
                ...
                <div className="question-actions">
                  ...
                  <button className="secondary-btn" onClick={resetPractice}><RotateCcw size={14} /> 重新从头开始</button>
                </div>
              </div>
            )}
          </section>
